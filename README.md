...
A small Node.js / Express demo app ([`server.mjs`](server.mjs)) used to showcase a **JFrog-secured CI/CD pipeline** on GitHub Actions: build → scan → publish → containerize, plus pull-request security scanning with **Frogbot v3** and **Snippet Detection**.
...
2. **Source scan** — `jf audit` with SCA + Contextual Analysis + Secrets + SAST + **Snippet Detection** (`--static-sca --snippet --sbom`). Results go to the JFrog Platform and a `scan-result-audit` artifact.
...
> **Snippet Detection** also requires the **Unified Security** entitlement on the platform. The base image is resolved through a **Curated** remote, so a first-ever pull may briefly 403 with *"curation on-demand in progress"* until the package is catalogued.
...
Snippet Detection for Frogbot is enabled **centrally** in the platform (Administration → Xray Settings → Indexed Resources → Git Repositories → *Configure* → enable **Snippet Detection**); Frogbot v3 picks up that Config Profile automatically by repository name.

> The older v2 workflow is kept disabled as `frogbot-pr.yaml.no` (the `.no` suffix stops GitHub from running it) to avoid duplicate PR comments.
...
### Scenario C — Snippet Detection (copyleft)

This repo ships a deliberate example: [`fastInvSqrt.mjs`](fastInvSqrt.mjs) is the *fast inverse square root* lifted verbatim from **Quake III Arena** (id Software), which is **GPL-2.0** — a copyleft license incompatible with this app's MIT license. It's wired into the API at `GET /api/fastinvsqrt/:value` so it isn't dead code.

Open a PR that adds/modifies this file (or paste another verbatim function from a known GPL/AGPL project). **Snippet Detection** matches the snippet against JFrog's open-source catalog — the iconic `0x5f3759df` constant is a strong fingerprint — and reports its GPL origin and license risk, both in the Frogbot PR comment and in the build pipeline's `jf audit --snippet` output.

> Detection requires Snippet Detection to be enabled (platform-side for Frogbot; `--snippet` for the build) and the source to be present in your tenant's snippet catalog.
